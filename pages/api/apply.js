export default async function handler(req, res) {
  ...
  if (chatlog.length < 80)
    return res.status(400).json({ message: 'O chatlog da cena precisa ser mais detalhado.' });

  const webhookUrl = process.env.DISCORD_WEBHOOK_URL;
  if (!webhookUrl) {
    console.log('[Allow-List] candidatura recebida (webhook não configurado):', req.body);
  ...
  try {
    ...
    if (!dr.ok) {
      console.error('Discord webhook error:', await dr.text());
